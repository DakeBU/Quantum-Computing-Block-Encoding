...
## Previous Run Directive: 2026-06-15 Post-Audit Theorem-Facing Finite Block/Projection Interface Prepared

This directive supersedes the theorem-facing finite block contract audit after
lower2 compiled it as non-promoting route memory:
...
The compiled audit records that the source-facing Fig. 4 transcript is
distinct from the active seven-gate backend, while
`oneTermRobinFiniteBlockCompositionContract 3` still uses
`oneTermRobinCircuitSemantics 3`.  It keeps product-to-coefficient,
normalized-block equality, LCU correctness, block projection, block
correctness, final extraction, oracle correctness, unitarity, and resource
claims false.

The root obligation remains open:

```lean
oneTermRobinGamma3ProductToCoefficientObligation 3 0 0
```

The new active leaf is a non-promoting theorem-facing finite block/projection
interface packet.  Middle prepared:

```text
proof-attempts/QBE-AUTO-002/theorem-facing-finite-block-projection-interface-middle-packet-20260615-0507.md
verifier-feedback/QBE-AUTO-002/theorem-facing-finite-block-projection-interface-middle-20260615-0507.json
```

The planned lower2 declarations are:
...
This packet should record the source-prepared projection target, the finite
block-composition contract, the compiled theorem-facing contract audit, the
theorem-facing and active backend transcript guards, and the exact remaining
obstruction: a corrected theorem-facing finite block/projection equality is
still missing before the root product-to-coefficient obligation can be
attempted.  It must not replace the paper circuit, mutate the active backend
...
1. lower1 validates the source map and keeps the focused branch fixed to system
   entry `(0,0)`, sparse slot `2`, source-prepared projection, branch basis
   `[32,32]`, signal block `[0,0]`, and normalizer `N_D*N_f*kappa`.
2. lower3 verifies the necessary conditions before lower2 edits Lean: the
   compiled audit exists, theorem-facing and active backend gate-list guards
   remain distinct, the finite block-composition contract still consumes
   `oneTermRobinCircuitSemantics 3`, the source-prepared projection target is
   the clean prepared entry, and all theorem flags remain false.
3. lower2 may edit only `QuantumBlockEncoding/RobinMatrix.lean` and only for
   the interface declarations above.  If they already exist, lower2 should
...
## Current Run Directive: 2026-06-15 Theorem-Facing Finite Block Contract Audit Prepared

This directive supersedes the finite normalized projection packet after lower2
compiled it as route memory:
...
The source-prepared projection bridge remains non-promoting compiled memory.
It keeps `FixedProductObligation.proved = false` and keeps normalized-block
equality, product-to-coefficient, LCU correctness, block projection, block
correctness, final extraction, oracle correctness, unitarity, and resource
claims false.
...
The new active leaf is a source-contract correction packet for the
theorem-facing finite block/projection contract.  Source audit found that
GHL2025 Theorem `theorem: 1 term robin`, Eq. `ROBIN clarified`, Fig.
`fig:1 term ROBIN`, and Definition `def:block-encoding` refer to the
theorem-facing Fig. 4 circuit, while the current finite block-composition
contract is wired through:
...
That backend is the active seven-gate component, not the full theorem-facing
Fig. 4 transcript:
...
Middle prepared the next source-proof translation packet:

```text
proof-attempts/QBE-AUTO-002/theorem-facing-finite-block-contract-audit-middle-packet-20260615-0448.md
verifier-feedback/QBE-AUTO-002/theorem-facing-finite-block-contract-audit-middle-20260615-0448.json
```
...
This declaration should be a non-promoting audit packet.  It should record the
theorem-facing Fig. 4 transcript guard, the active backend transcript guard,
the current finite block-composition contract wiring, the compiled normalized
projection bridge, and the false theorem flags.  It must not replace the
paper circuit or promote normalized-block equality, LCU correctness, block
projection, block correctness, final extraction, oracle correctness, unitarity,
resource claims, or product-to-coefficient closure.

Lower-agent split:

1. lower1 validates the source dependency map and keeps the branch fixed to
   system entry `(0,0)`, sparse slot `2`, source-prepared projection, branch
   basis `[32,32]`, and normalizer `N_D*N_f*kappa`.
2. lower3 verifies the necessary conditions before lower2 edits Lean: the
   theorem-facing and active backend gate-list guards are distinct, the finite
   block-composition contract still consumes `oneTermRobinCircuitSemantics 3`,
   the normalized projection bridge is compiled with all theorem flags false,
   and raw backend expansion plus `SignalEntryFold` equivalents remain
   forbidden.
3. lower2 may edit only `QuantumBlockEncoding/RobinMatrix.lean` and only for
   the audit declarations above.  If they already exist, lower2 should make no
   Lean edit and log `error_class=stale_leaf`.
...
## Current Run Directive: 2026-06-15 Finite Normalized Projection Packet Prepared

This directive supersedes the 2026-06-15 conditional normalizer bridge closure
after middle prepared the next source-proof translation packet:

```text
proof-attempts/QBE-AUTO-002/finite-normalized-projection-middle-packet-20260615-0419.md
verifier-feedback/QBE-AUTO-002/finite-normalized-projection-middle-20260615-0419.json
```

The conditional bridge remains compiled route memory and must not be
reassigned:
...
The next active proof-DAG leaf is the source-prepared finite normalized
projection bridge. The planned lower2 declaration is:

```lean
oneTermRobinGamma3BoundarySourcePreparedNormalizedProjectionBridge_n3
```

This declaration should be a non-promoting packet and transcript tying the
compiled conditional normalizer bridge, the finite projection product bridge,
the source-prepared product/projection obligation, and
`oneTermRobinFiniteBlockCompositionContract 3` to the fixed product map. It
must keep `FixedProductObligation.proved = false` and must not promote
normalized-block equality, LCU correctness, block projection, block
...
1. lower1 validates the source proof map in the finite-normalized-projection
   middle packet and keeps the branch fixed to system entry `(0,0)`, sparse
   slot `2`, source-prepared projection, and normalizer `N_D*N_f*kappa`.
2. lower3 verifies the necessary conditions before lower2 edits Lean: signal
   block indices are `[0,0]`, branch basis is `[32,32]`, the conditional
   bridge consumes the source-prepared projection entry, and raw backend
   expansion plus `SignalEntryFold` equivalents remain forbidden.
3. lower2 may edit only `QuantumBlockEncoding/RobinMatrix.lean` and only for
   the declaration above. If it already exists, lower2 should make no Lean
   edit and log `error_class=stale_leaf`.
...
## Current Run Directive: 2026-06-15 Conditional Normalizer Bridge Closed

This directive supersedes the 2026-06-15 product-to-coefficient/normalizer
proof-translation packet after lower2 compiled the assigned conditional bridge.
The following declarations are now route memory:
...
The bridge is conditional under explicit hypotheses for the sparse-preparation
clean column, boundary entry convention, and coefficient normalizers. It keeps
the fixed root obligation and downstream theorem flags false. The root
...
The earlier guard also remains compiled route memory and must not be reassigned:

```lean
oneTermRobinGamma3BoundarySourcePreparedProjectionSlot2Product_feedsFixedProductMap_n3
```

The raw backend-expansion statement, the equivalence to `SignalEntryFold`, and
all propositions still equivalent to that raw fold remain forbidden by the
compiled finite counterexample:

```lean
oneTermRobinGamma3BoundaryBackendExpansionStatement_not_n3
```

Current source-facing closure artifacts:
...
verifier-feedback/QBE-AUTO-002/product-to-coefficient-normalizer-middle-postlower-20260615-034853.json
```

Next lower-agent split:

1. lower1 or middle prepares the next source-proof translation packet for the
   remaining finite normalized-block/projection bridge feeding
   `oneTermRobinGamma3ProductToCoefficientObligation 3 0 0`.
2. lower2 has no new Lean theorem target until that packet names exactly one
   declaration. If run before then, it should make no Lean edit and report the
   compiled conditional bridge as route memory.
3. lower3 should verify necessary conditions for the next finite
   normalized-block/projection bridge and keep `product_to_coefficient=false`
   until a theorem closes the root obligation.

No oracle, `H_W`, boundary `R_y`, LCU/block composition, oracle correctness,
unitarity, normalized block equality, block correctness, final extraction,
resource, normalizer-free, or product-to-coefficient flag is promoted by the
compiled conditional bridge.

## Retired Run Directive: 2026-06-15 Source-Contract Route Retarget Guard

This directive supersedes the 2026-06-15 backend-expansion correction audit
after lower1 and lower3 closed the unchanged raw backend-expansion target as a
source-contract gap with a finite counterexample.  The raw proposition remains
route memory only:
...
Do not assign it, or any proposition still equivalent to `SignalEntryFold`, as
a theorem-facing lower2 target.  The paper-backed source-facing route consumes
the compiled source-prepared projection/product bridge:

```lean
oneTermRobinGamma3BoundarySourcePreparedProjection_slot2_to_projectedBranchProduct_n3
```

Middle has named and compiled the route-retarget guard that attaches this
source-prepared slot-`2` product equality to the fixed product map while all
product, normalized-block, LCU, block, and final-extraction flags remain false:

```lean
oneTermRobinGamma3BoundarySourcePreparedProjectionSlot2Product_feedsFixedProductMap_n3
```

This guard does not prove:

```lean
oneTermRobinGamma3ProductToCoefficientObligation 3 0 0
```

The fixed product-to-coefficient obligation remains open.  The next lower2
work, if any lower2 is run on this directive, is only to verify that the guard
above is present and compiled.  If the guard is already present, lower2 should
make no Lean edit and log `error_class=stale_leaf`.  Future theorem-facing
work must be a new coefficient/normalizer proof packet for
`oneTermRobinGamma3ProductToCoefficientObligation 3 0 0`; it must not revive
the refuted raw backend-expansion statement.

Lower-agent split:

1. lower1 uses
   `proof-attempts/QBE-AUTO-002/backend-expansion-route-retarget-middle-packet-20260615-030358.md`
   as the source-dependency/proof-DAG map.  It should not search for another
   raw branch-sum target.
2. lower3 records typed feedback for the retarget guard:
   `source_correspondence_ok=true` for the source-prepared route,
   `finite_matrix_ok=true` for the guard shape, `closed_theorem_ok=true` for
   the guard itself, and `product_to_coefficient=false` in notes.
3. lower2 may edit only `QuantumBlockEncoding/RobinMatrix.lean` and only if
   the guard declaration above is absent.  It must prove exactly that guard by
   reusing the compiled slot-`2` source-prepared projection/product theorem and
   false-flag records.  If the guard is present, lower2 must make no edit.

No oracle, `H_W`, boundary `R_y`, LCU/block composition, oracle correctness,
unitarity, normalized block equality, block correctness, final block
extraction, normalizer, or product-to-coefficient flag is promoted by this
retarget guard.

## Current Run Directive: 2026-06-15 Backend-Expansion Correction Audit

This directive supersedes the 2026-06-15 source-prepared
projection/product composite directive after lower work closed the composite
leaf in `QuantumBlockEncoding/RobinMatrix.lean`.

The source-prepared product/projection packet, the two bridge leaves, and the
composite are compiled route memory:
...
The upper handoff in
`runs/20260615-022953-QBE-AUTO-002-cycle01/dialogue.md` selects the next
frontier as the projection/backend-expansion leaf feeding the fixed
`oneTermRobinGamma3ProductToCoefficientObligation 3 0 0`.  Middle must not
assign blind tactic search on the raw target, because the current Lean
interface is already refuted:
...
The raw backend-expansion proposition is still useful as route memory because
it is typed and equivalent to the full signal-zero entry fold:

```lean
oneTermRobinGamma3BoundaryBackendExpansionStatement_equivUnitaryEntryFold_n3
```

but lower work must refine the Lean contract before any theorem-facing
branch-sum proof is attempted.  The local TeX source path
`outer_papers/quantum/GHL2025/main.tex` is absent in this checkout; public
source anchors remain the maintained GHL2025 proof notes, Fig. `fig:1 term
ROBIN` visual audit, cited-results rows, Eq. `ROBIN clarified`, Definition
`def:block-encoding`, and Theorem `theorem: 1 term robin`.

Next lower work:

1. lower1 writes a source-dependency/proof-DAG correction packet for the
   projection/backend-expansion frontier.  It must explain whether the paper
   supports a corrected source-facing branch-sum proposition, or whether this
   is a source-contract gap.
2. lower3 records a necessary-condition verifier packet for the raw target:
   the existing all-one selected-branch counterexample means the unchanged
   `backendExpansionStatement` is not a valid lower2 theorem target.
3. lower2 does not try to prove the refuted proposition unchanged.  If lower2
   is run before lower1/lower3 finish the correction, it should make no Lean
   edit and log `error_class=finite_matrix_counterexample`.  After the
   correction packet exists, lower2 may edit only
   `QuantumBlockEncoding/RobinMatrix.lean` and only for the single corrected
   Lean leaf named by that packet.

Do not use the two diagnostic `sorry` declarations, do not revive the old
H-free evaluated fold as a theorem-facing route, and do not promote
product-to-coefficient, normalizer, LCU/block composition, oracle correctness,
unitarity, block correctness, or final block-extraction flags.

## Current Run Directive: 2026-06-15 Source-Prepared Projection/Product Bridge After Packet

This directive supersedes the 2026-06-15 source-prepared product/projection
packet directive and the older 2026-06-13 finite-path feeder immediate
directive.  The packet is now compiled and lower3-verified:
...
Retire that packet as a lower2 target.  It remains route memory proving only
that the source-prepared Fig. `fig:1 term ROBIN` clean projection, the fixed
product obligation `oneTermRobinGamma3ProductToCoefficientObligation 3 0 0`,
and the finite product/projection bridge are named together while all
product/LCU/block/final flags stay false.

The active objective is the source-prepared projection-to-projected-product
evaluated bridge.  Do not use the refuted backend-expansion parent, the H-free
evaluated fold as the main route, or the old active selected-slot feeder.

First lower2 target, if absent:
...
1. lower1 writes the source-proof DAG for the bridge, explicitly separating
   the source-prepared clean projection from the H-free backend fold and
   naming where `hUniform` and `hentry` enter.
2. lower3 checks the finite shape: source-prepared clean index `0`, focused
   sparse slot `2`, branch full index `32`, fixed product `3 0 0`, no
   backend-expansion dependency, no diagnostic `sorry` dependency, and no
   theorem flag promotion.
3. lower2 proves exactly one bridge leaf.  Start with
   `oneTermRobinGamma3BoundarySourcePreparedProjection_to_backendFold_n3` if it
   is not already present; otherwise prove
   `oneTermRobinGamma3BoundaryBackendFold_to_slot2ProjectedProduct_n3`.
...
oneTermRobinGamma3BoundaryEvaluatedBackendFoldStatement_n3 env as the main target
...
The fixed product-to-coefficient theorem, normalizer algebra, LCU/block
composition, oracle correctness, unitarity, and final block-extraction theorem
remain later obligations.

## Current Run Directive: 2026-06-15 Source-Prepared Product/Projection Packet

This directive supersedes the `2026-06-14 Pro-Assessed Source-Prepared Target
Repair` directive below for the next 6h batch.  Full assessment:

```text
proof-attempts/QBE-AUTO-002/source-prepared-product-projection-plan-20260615-pro.md
```

ChatGPT Pro's latest response is accepted as the next proof-DAG retarget.  The
next lower target is not the H-free backend fold, not direct row-`0` to selected
slot `2`, and not the active/prepared diagnostic comparison.  The next target
is a source-prepared product/projection route starting from the clean projection
of the full Fig. `fig:1 term ROBIN` object:
...
backend fold.  Therefore these names are diagnostic wrappers.  Do not assign
them as the main lower2 target in this batch.

The prepared clean projection bridge is already compiled:
...
The first concrete lower2 target is a bookkeeping/proof-DAG packet, not the full
product-to-coefficient equality:
...
After the packet compiles, the first mathematical leaf is:

```lean
oneTermRobinGamma3BoundarySourcePreparedProjection_slot2_to_projectedBranchProduct_n3
```

or a smaller decomposition through:
...
The second of these is the real finite projection/product bridge.  It must not
use `backendExpansionStatement`.

Next lower work:

1. lower1 writes the source-proof DAG for the packet and the first projection
   leaf, explicitly tied to Fig. 4, Definition `def:block-encoding`, and Eq.
   `ROBIN clarified`.
2. lower3 checks that the packet has no dependency on the refuted
   `backendExpansionStatement`, no diagnostic `sorry` dependency, and no hidden
   promotion of product/LCU/block/final flags.
3. lower2 first adds only the packet and transcript theorem.  If that compiles
   quickly, lower2 may then try
   `oneTermRobinGamma3BoundarySourcePreparedProjection_to_backendFold_n3` as a
   direct wrapper over the compiled source-prepared bridge.  Do not spend this
   batch unfolding giant matrices or proving raw `Coeff` constructor equality.

Scheduling rule for this batch: run one upper director pass, middle
source-correspondence/memory refresh, lower agents in parallel, one reviewer
pass after lower edits or blocker report, then one Chinese summary and one
ChatGPT Pro prompt at the end.  Avoid another upper/middle/reviewer-only batch.
...
The normalizer algebra assumptions and the `R_y` boundary coefficient bridge
belong to later local product-to-coefficient leaves, not to this packet.
...
## Current Run Directive: 2026-06-13 Backend-Fold Obstruction Retarget For Run 20260613-182230

This directive implements the upper handoff in
`runs/20260613-182230-QBE-AUTO-002-cycle01/dialogue.md`.

Retire the all-environment H-free evaluated backend fold as an active lower2
proof target:
...
Lower3 has already recorded a finite all-one environment where the selected
slot-`2` contribution evaluates nonzero.  Therefore the old root fold is
classified as `finite_matrix_counterexample`, not as a tactic gap.  The older
direct row-`0` to selected slot-`2` feeder remains retired as
...
1. lower1 writes the source/DAG obstruction map from Eq. `ROBIN clarified`,
   Fig. `fig:1 term ROBIN`, Definition `def:block-encoding`, and the lower3
   finite diagnostic to one exact lower2 obstruction leaf.
2. lower3 verifies that the old root remains a finite counterexample, that the
   direct row-`0` to slot-`2` feeder remains `shape_or_register_gap`, and that
   `hUniform` stays downstream-only.
3. lower2 proves exactly one formal obstruction leaf in
   `QuantumBlockEncoding/RobinMatrix.lean`: preferred
   `oneTermRobinGamma3BoundarySelectedSlotContribution_allOne_nonzero_n3`,
   fallback
   `oneTermRobinGamma3BoundaryEvaluatedBackendFold_forces_selectedSlotContribution_zero_n3`.

Lower2 must not prove the retired root fold, revive
`oneTermRobinGamma3BoundaryActiveSelectedSlotEvalFeeder_n3`, add `hUniform` to
the H-free statement, use sorry-guarded raw diagnostics as theorem closure, or
...
## Current Run Directive: 2026-06-13 Branch-Correct Source-Prepared Backend Fold For Run 20260613-180059

This directive implements the upper handoff in
`runs/20260613-180059-QBE-AUTO-002-cycle01/dialogue.md`.

The ChatGPT Pro finite-path deployment packet remains trial memory, but the
direct H-free feeder

```lean
oneTermRobinGamma3BoundaryActiveSelectedSlotEvalFeeder_n3 env
```

remains retired as `shape_or_register_gap`: it compares active full index `0`
with selected sparse slot `2` / full index `32`.

The already compiled route-wiring leaves are stale as lower targets:
...
The current branch-correct local field is the evaluated backend fold:
...
The sparse-register preparation contract remains explicit and downstream:
...
1. lower1 maps Eq. `ROBIN clarified`, Fig. `fig:1 term ROBIN`, Definition
   `def:block-encoding`, and the Fig. 4 audit to the full evaluated backend
   fold and names one lower2 feeder into that fold.
2. lower3 checks that the target is the full branch sum, that source-prepared
   recovery uses `hUniform` only after the fold, and that the direct row-`0` to
   slot-`2` route remains rejected.
3. lower2 proves exactly one evalWith-level branch-correct backend-fold leaf or
   one strict feeder into it.  Lower2 edits only
   `QuantumBlockEncoding/RobinMatrix.lean` unless middle explicitly authorizes
   a small generic wrapper.

Lower2 must not add `hUniform` to the H-free fold, revive the selected-slot
feeder, prove arbitrary-`H` closure without a source-backed finite-composition
bridge, use sorry-guarded raw declarations as theorem closure, or promote
oracle, `H_W`, `R_y`, LCU, unitarity, block-projection, normalizer,
product-to-coefficient, block-correctness, or final-extraction flags.

## Current Run Directive: 2026-06-13 Source-Prepared Finite Composition For Run 20260613-174250

This directive implements the upper handoff in
`runs/20260613-174250-QBE-AUTO-002-cycle01/dialogue.md`.

The ChatGPT Pro finite-path feeder deployment packet remains part of trial
memory, but lower3 rejected the direct strict H-free feeder
...
two-path tail-kill diagnostic, while the selected source contribution is sparse
slot `2` at full index `32`.  Do not reassign this theorem unless middle first
changes the source-backed active index contract.

The prepared-projection restatement leaf is already compiled and is stale as a
lower target:
...
The current theorem-facing local field is the source-prepared finite
composition equality.  Accepted equivalent targets are:
...
The sparse-register preparation contract remains explicit and is used only for
downstream backend recovery:
...
1. lower1 maps GHL2025 Eq. `arbitrary sparcity`, Eq. `ROBIN clarified`, Fig.
   `fig:1 term ROBIN`, Definition `def:block-encoding`, and the Fig. 4 audit
   to the active/prepared field, the cached entry target, and the compiled
   downstream backend recovery bridges.
2. lower3 checks finite path/support shape for the source-prepared field and
   rejects any revived standalone active row-`0` to selected slot-`2` feeder as
   `shape_or_register_gap`.
3. lower2 proves exactly one source-prepared finite-composition leaf or one
   strict feeder into that leaf.  Lower2 edits only
   `QuantumBlockEncoding/RobinMatrix.lean` unless middle explicitly authorizes
   a small generic `CircuitSemantics` wrapper.

Lower2 must not reprove the compiled restatement leaf, revive the H-free
selected-slot feeder, use the sorry-guarded raw declarations as theorem
closure, add assumptions to the paper theorem, or promote oracle, `H_W`,
`R_y`, LCU, unitarity, block-projection, normalizer, product-to-coefficient, or
final-extraction flags.

## Current Run Directive: 2026-06-13 Prepared Projection Restatement For Run 20260613-172255

This directive implements the latest upper handoff in
`runs/20260613-172255-QBE-AUTO-002-cycle01/dialogue.md`.  If focused context
replays the ChatGPT Pro strict H-free feeder override, treat that route as
stale for this run.  The strict H-free feeder

```lean
oneTermRobinGamma3BoundaryActiveSelectedSlotEvalFeeder_n3 env
```

remains retired as `shape_or_register_gap`: the left side is the active
full-basis row `0` entry, while the selected source contribution is sparse slot
`2` at full index `32`.

The current theorem-facing route must be restated around the prepared
projection entry and the active/prepared composition field.  The prepared-side
bridge is already compiled:
...
The remaining local field is the active/prepared entry comparison carried by

```lean
oneTermRobinGamma3BoundaryActivePreparedEntryTarget_n3 H
```

or, at evaluation level, the proposition exposed by
...
The sparse-register preparation contract must stay explicit:
...
1. lower1 maps GHL2025 Eq. `arbitrary sparcity`, Eq. `ROBIN clarified`, Fig.
   `fig:1 term ROBIN`, Definition `def:block-encoding`, and the Fig. 4 audit
   to `preparedProjectionEntry`, `oneTermRobinGamma3BoundaryActivePreparedEntryTarget_n3`,
   and the compiled prepared-entry backend bridge.
2. lower3 checks that the prepared projection entry uses the full
   source-prepared route with both `H_W` sides, while the active seven-gate
   row remains diagnostic-only and H-free.
3. lower2 proves exactly one source-shaped restatement or route leaf, not the
   root theorem.  Preferred leaf:
...
Lower2 must not prove or revive the H-free selected-slot feeder, must not use
the sorry-guarded raw declarations as theorem closure, must not add assumptions
to the paper theorem, and must not promote oracle, `H_W`, `R_y`, LCU,
unitarity, block-projection, normalizer, or final-extraction flags.

## Current Run Directive: 2026-06-13 Source-Prepared Contract Retarget For Run 20260613-170242

This directive implements the upper synthesis in
`runs/20260613-170242-QBE-AUTO-002-cycle01/dialogue.md`.

The previously assigned column-`0` bridge is no longer open:

```lean
oneTermRobinGamma3BoundaryEvalGateMatricesColumn0Entry_eq_sevenGateMatrix_n3
```

compiled as a diagnostic guard.  It proves only that the H-free active
`evalGateMatrices[0,0]` entry agrees with the explicit seven-gate `[0,0]`
entry.  Together with
...
it shows the active H-free column-`0` route is a zero diagnostic path.  This
does not prove the selected sparse slot contribution, does not close the
one-term Robin theorem, and does not promote any oracle, `H_W`, `R_y`, LCU,
unitarity, block-projection, normalizer, or final-extraction flag.

The strict H-free selected-slot feeder remains retired:

```lean
oneTermRobinGamma3BoundaryActiveSelectedSlotEvalFeeder_n3 env
```

It compares active full-basis index `0` with selected sparse slot `2` / full
index `32`; classify every revival as `shape_or_register_gap`.

The current source-faithful route is still
`source_prepared_projection_summation_correction`, now retargeted to the
prepared singleton clean entry in the full Fig. `fig:1 term ROBIN` route.  The
paper-facing contract keeps the sparse-register preparation hypothesis
explicit:
...
1. lower1 maps GHL2025 Eq. `arbitrary sparcity`, Eq. `ROBIN clarified`, Fig.
   ...
   audit to the prepared singleton clean-entry interface.
2. lower3 records finite path/support feedback for active full index `0`,
   selected slot `2` / full index `32`, the two `R_y` tails, `O_f` tail-kill,
   and the presence of both `H_W` sides in the source-prepared route.
3. lower2 proves exactly one small Lean leaf.  Preferred first guard leaf:
   `oneTermRobinGamma3BoundaryActiveEvalColumn0_zero_n3 (env : String -> Rat)`
   from the compiled column-`0` bridge and
   `oneTermRobinGamma3BoundarySevenGateColumn0Eval_zero_n3`.  If middle/lower1
   selects the theorem-facing leaf instead, target only the uncast prepared
   sparse-clean comparison exposed by
   `oneTermRobinGamma3BoundarySourcePreparedProjectionTarget_activeEval_iff_uncastPreparedSparseCleanEntry_n3
   H env`.

Lower2 must not use the sorry-guarded raw declarations
...
closure, must not add `hUniform` to the retired H-free feeder, and must not
change the paper circuit, oracle contracts, gate order, normalizer, or theorem
assumptions.
...
## EOF Mirror: 2026-06-15 Conditional Normalizer Bridge Closed

The product-to-coefficient/normalizer bridge packet is now accepted route
memory. Lower2 compiled:
...
The compiled bridge is conditional under explicit hypotheses and keeps the
fixed product obligation and downstream theorem flags false. The root remains
open:
...
The stale guard remains route memory:
...
1. lower1 or middle prepares a new source-proof translation packet for the
   remaining finite normalized-block/projection bridge feeding
   `oneTermRobinGamma3ProductToCoefficientObligation 3 0 0`.
2. lower2 has no new Lean theorem target until that packet names exactly one
   declaration.
3. lower3 verifies necessary conditions for the next finite
   normalized-block/projection bridge and keeps `product_to_coefficient=false`
   until the root theorem closes.
...
## Current Run Directive: 2026-06-13 Post-Strict-Feeder Source-Prepared Active-Entry Frontier

This directive supersedes the post-bridge evaluated-backend-fold packet for the
current run `20260613-022313-QBE-AUTO-002-cycle01`.  The latest upper
source-contract audit selects the theorem-facing source-prepared route with
both $H_W^{(\kappa)}$ sides represented through the existing
`oneTermRobinGamma3BoundaryHWKappaUniformColumnAllSlotsStatement_n3 H`
contract.  The standalone H-free `semantic_eval_product_bridge` is diagnostic
memory only unless a source-prepared bridge prevents the slot/register drift.

Lower 2 has already compiled

```lean
oneTermRobinGamma3BoundaryActivePreparedEntryTarget_iff_rawEntryPreparedSandwichField_n3
```

which proves that, under `hUniform`, `PreparedEntry(H)` is equivalent to the
raw prepared-sandwich field.  That theorem is accepted route wiring and is now
retired as a lower target.  It does not prove `PreparedEntry(H)`, the raw
prepared-sandwich field, the uncast active/prepared target, the evaluated
backend fold, or the one-term theorem.

Next lower work:

1. Lower 1 may append only a narrow postscript naming the compiled feeder above
   and retiring it.
2. Lower 2 edits only `QuantumBlockEncoding/RobinMatrix.lean`.
3. Lower 2 should prove exactly
   `(oneTermRobinGamma3BoundaryRawEntryPreparedSandwichCircuitField_n3 H).rawEntryPreparedSandwichStatement`,
   ...
   `oneTermRobinGamma3BoundaryUncastActivePreparedCompositeEvalStatement_n3 H env`,
   `(oneTermRobinGamma3BoundarySourcePreparedProjectionTarget_n3 H env).activeToPreparedSingletonEvalStatement`,
   or one strictly smaller source-shaped theorem that directly feeds one of
   these statements.
4. Retire the compiled feeder, the post-feeder sparse-clean-to-fold bridge,
   the strict prepared-sparse feeder, finite reduction guards, H-free
   active-selected diagnostics, backend slot vanish/support work, raw `Coeff`
   constructor equalities, branch-sum wrappers, and compiled bridge
   rediscovery.
5. If a lower route again compares the H-free active slot directly to the
   gamma3 selected backend slot without the prepared sandwich, record
   `shape_or_register_gap`.  If the route is source-shaped but blocked by Lean
   finite matrix algebra, record `symbolic_bridge_gap`.

The gate remains `python3 tools/qbe.py check`, then `lake build`, then
`lake build Tests`.  The first-case-study one-term theorem remains open, and
no oracle, `H_W`, `R_y`, LCU, block-projection, normalized-equality,
product-to-coefficient, circuit-unitarity, block-correctness,
final-extraction, normalizer, or external primitive flag is promoted.

## Current Run Directive: 2026-06-13 Prepared Clean-Entry Source-Prepared Frontier

This directive supersedes the post-strict-feeder packet for the current run
`20260613-024914-QBE-AUTO-002-cycle01`.  The latest lower3
necessary-condition feedback observes that

```lean
oneTermRobinGamma3BoundaryRawEntryPreparedSandwichField_iff_preparedCleanEntry_n3
```

has compiled.  That source-shaped feeder proves only that the raw
prepared-sandwich field is equivalent to the concrete prepared sparse-matrix
clean-entry equality.  It does not prove the prepared clean-entry equality,
the cached active/prepared entry equality, the theorem-facing
source-prepared projection field, the evaluated backend fold, or the
first-case-study one-term theorem.

The next precise lower2 target is the prepared clean-entry equality:
...
Equivalent acceptable target:

```lean
(oneTermRobinGamma3BoundaryActivePreparedEntryTarget_n3 H).entryEqualityStatement
```

Lower2 may instead prove one strictly smaller source-shaped theorem that
directly feeds one of these two statements.  The compiled feeder above,
...
and the earlier raw/entry feeders are route wiring only and are retired as
lower targets.

Retire the standalone H-free `semantic_eval_product_bridge`, compiled
source-shaped feeder rediscovery, strict prepared-sparse feeders, sparse-clean
to fold bridges, finite active/prepared reduction guards, backend slot
vanish/support work, raw `Coeff` constructor equalities, and branch-sum
wrappers.  If a route compares the H-free active slot directly to the gamma3
selected backend slot without the prepared sandwich, record
`shape_or_register_gap`.  If the route is source-shaped but blocked by Lean
finite matrix algebra, record `symbolic_bridge_gap`.

Lower2 edits only `QuantumBlockEncoding/RobinMatrix.lean`.  The gate remains
`python3 tools/qbe.py check`, then `lake build`, then `lake build Tests`.
The first-case-study one-term theorem remains open.  No oracle, `H_W`, `R_y`,
LCU, block-projection, normalized-equality, product-to-coefficient,
circuit-unitarity, block-correctness, final-extraction, normalizer, or
external primitive flag is promoted.

## Current Run Directive: 2026-06-13 Post-Lower2 Active/Prepared Composition Interface Frontier

This directive supersedes the prepared clean-entry wording for the current run
`20260613-033618-QBE-AUTO-002-cycle01`.  Lower2's blocked packet
`proof-attempts/QBE-AUTO-002/prepared-clean-entry-lower2-blocked-20260613-033219.md`
made no Lean edit and found no counterexample certificate.  It confirmed that
the compiled obstruction handle

```lean
oneTermRobinGamma3BoundaryPreparedCleanEntryLeaf_obstruction_n3 H
```

only names the same open proposition and must remain retired as a lower target.
The active leaf is now the existing active/prepared composition interface:
...
Lower2 may instead prove one strictly smaller source-shaped finite
`CircuitMatrixSemantics` theorem that directly feeds this active/prepared
composition field.  The smaller theorem must not be another obstruction
record, equivalence wrapper, raw `Coeff` constructor equality, backend-slot
vanish/support lemma, branch-sum wrapper, or standalone H-free selected-slot
diagnostic.
...
| Declaration | Status |
|---|---|
| `oneTermRobinGamma3BoundaryActivePreparedCompositionFieldTarget_n3 H` | compiled field record exposing the exact missing composition theorem |
| `oneTermRobinGamma3BoundaryActivePreparedCompositionFieldTarget_n3_transcript H` | compiled transcript with `activePreparedEntryEqualityProved = false` |
| `oneTermRobinGamma3BoundaryActivePreparedCompositionFieldTarget_interfaceStatement_n3 H` | compiled equivalence between field and interface statements |
...
| Lower profile | Required behavior |
|---|---|
| lower 1: natural-language proof architect | Reuse the prior proof-DAG addendum. Append only a narrow postscript if needed, naming the active composition interface leaf and retiring the obstruction handle plus feeders. |
| lower 2: Lean implementation worker | Edit only `QuantumBlockEncoding/RobinMatrix.lean`. Prove the active composition interface leaf above, an equivalent target above, or one strict finite composition theorem feeding it directly. |
| lower 3: necessary-condition verifier | Check source-shaped active/prepared composition routes only. Record `symbolic_bridge_gap` for source-shaped finite algebra blockers and `shape_or_register_gap` for standalone H-free reassignment. |

The source-dependency classification is internal QBE-local finite matrix
composition.  GHL2025 Eq. `arbitrary sparcity` and the Shukla-Vedula row remain
contract-only through `oneTermRobinGamma3BoundaryHWKappaUniformColumnAllSlotsStatement_n3 H`;
do not recursively formalize that external state-preparation result in this
packet.

The gate remains `python3 tools/qbe.py check`, then `lake build`, then
`lake build Tests`.  The first-case-study one-term theorem remains open.  No
...
`20260613-052836-QBE-AUTO-002-cycle01`.  The configured local GHL2025 TeX
archive was unavailable, so lower work must rely on the checked-in source maps
and proof ledgers rather than a new line-level TeX reread.  Public anchors
remain GHL2025 Eq. `arbitrary sparcity`, Eq. `ROBIN clarified`, Fig. `fig:1
term ROBIN`, and Definition `def:block-encoding`.

The active lower2 target remains:

```lean
(oneTermRobinGamma3BoundarySourcePreparedProjectionTarget_n3 H env).activeToPreparedSingletonEvalStatement
```

Equivalent accepted targets remain:
...
Lower2 edits only `QuantumBlockEncoding/RobinMatrix.lean`.  Lower2 may prove
one strict finite source-shaped theorem directly feeding these statements, but
must not add `Uniform(H)` to the current arbitrary-`H` target.  If `Uniform(H)`
is necessary, record `source_translation_gap` and return the target to middle
for restatement.  Standalone H-free evaluated-fold reassignment remains
`shape_or_register_gap`.
...
## Current Run Directive: 2026-06-13 Source-Translation Correction For Active/Prepared Interface

This directive supersedes the broad active/prepared composition-interface
packet for run `20260613-040302-QBE-AUTO-002-cycle01`.  The active interface

```lean
(oneTermRobinGamma3BoundaryActivePreparedCompositionFieldTarget_n3 H).activeEntryStatement
```

still names the missing equality between the active signal-zero entry and the
prepared sparse-matrix clean-clean entry.  Lower1's postscript and the upper
source audit refine the classification: the source-backed route is conditional
on the paper's sparse-preparation operation $H_W^{(\kappa)}$ and the existing
...
from GHL2025 Eq. `arbitrary sparcity` and the Shukla--Vedula cited row.  An
unconditional theorem for arbitrary `H` is not source-justified unless QBE first
proves a finite matrix-entry independence theorem showing that the selected
prepared clean entry does not depend on the unconstrained entries of `H`.

The next lower work is therefore one of:
...
or one strictly smaller source-shaped finite `CircuitMatrixSemantics` lemma
that either proves the active/prepared matrix-entry equality directly or proves
the required arbitrary-`H` independence fact.  If the worker cannot produce
that finite theorem, record `source_translation_gap`.  If the worker reassigns
the standalone H-free seven-gate/backend selected-slot route, record
`shape_or_register_gap`.

Retired targets remain retired: obstruction handles, feeder equivalences,
compiled bridge rediscovery, backend slot vanish/support work, raw `Coeff`
constructor equalities, branch-sum wrappers, H-free active-selected
diagnostics, and any route that adds `hUniform` as a new hypothesis to the
existing arbitrary-`H` target instead of proving or restating a source-backed
Lean contract through middle.

Lower2 edits only `QuantumBlockEncoding/RobinMatrix.lean`.  The gate remains
`python3 tools/qbe.py check`, then `lake build`, then `lake build Tests`.  The
first-case-study one-term theorem remains open.  No oracle, `H_W`, `R_y`, LCU,
block-projection, normalized-equality, product-to-coefficient,
circuit-unitarity, block-correctness, final-extraction, normalizer, or external
primitive flag is promoted.

## Current Run Directive: 2026-06-13 Current-Run Evaluated Backend Fold Frontier

This directive supersedes the source-translation correction packet for run
`20260613-042537-QBE-AUTO-002-cycle01`.  The source audit remains valid:
GHL2025 Eq. `arbitrary sparcity` and the Shukla--Vedula cited result justify
only the clean-column contract

```lean
oneTermRobinGamma3BoundaryHWKappaUniformColumnAllSlotsStatement_n3 H
```

for the paper sparse-preparation operation.  The unconditional arbitrary-`H`
active/prepared interface is no longer the default lower target.  The compiled
clean-column congruence theorem is useful support, but it does not prove the
active/prepared equality or the first-case-study theorem.

The active local root is now:

```lean
oneTermRobinGamma3BoundaryEvaluatedBackendFoldStatement_n3 env
```

Lower2 may instead prove one strictly smaller finite `Coeff.evalWith` product
or projection lemma that directly feeds this statement through the existing
bridge
...
or through the source-prepared sparse-clean bridge under the existing
`hUniform` contract.  The exact active/prepared sparse-clean equality remains
an allowed equivalent route only when the clean-column contract is kept
explicit through middle; lower agents must not add a new theorem hypothesis or
change the paper circuit.

Compiled support to reuse and retire as lower targets:

| Declaration | Status |
|---|---|
| `oneTermRobinGamma3BoundaryUncastPreparedSparseCleanEntryEval_iff_evaluatedBackendFold_n3 H env hUniform` | compiled route wiring; retired |
| `oneTermRobinGamma3BoundaryEvaluatedBackendFoldStatement_iff_uncastActiveCircuitEntryEval_n3 env` | compiled uncast active-entry bridge; reuse |
| `oneTermRobinGamma3BoundaryEvaluatedBackendFoldStatement_of_activePreparedEval_n3 H env hUniform` | compiled conditional recovery from source-prepared eval; reuse |
| `oneTermRobinGamma3BoundaryPreparedCircuitSparseMatrix_cleanEntry_congr_cleanColumn_n3` | compiled partial clean-column support; not active/prepared closure |
| `oneTermRobinGamma3BoundaryUnitaryEntry_eq_backendFold_n3` and `oneTermRobinGamma3BoundaryEvalGateMatrices_eq_sevenGateMatrix_n3` | diagnostic `sorry` route; not theorem closure |

Lower-agent split:

| Lower profile | Required behavior |
|---|---|
| lower 1: natural-language proof architect | Append only a narrow postscript naming `evaluated_backend_fold_leaf` and retiring the arbitrary-`H` direct interface as the default target. |
| lower 2: Lean implementation worker | Edit only `QuantumBlockEncoding/RobinMatrix.lean`; prove `oneTermRobinGamma3BoundaryEvaluatedBackendFoldStatement_n3 env` or one strict finite eval/product lemma that directly feeds it. |
| lower 3: necessary-condition verifier | Check finite `n = 3` evaluated product/projection shape and record typed feedback; reject H-free selected-slot reassignment as `shape_or_register_gap` unless a new source-faithful bridge is named. |

Retired routes remain retired: obstruction handles, feeder equivalences,
compiled bridge rediscovery, backend slot vanish/support work, raw `Coeff`
constructor equalities, branch-sum wrappers, and H-free active-selected
diagnostics.  The gate remains `python3 tools/qbe.py check`, then
`lake build`, then `lake build Tests`.  The first-case-study one-term theorem
remains open.  No oracle, `H_W`, `R_y`, LCU, block-projection,
normalized-equality, product-to-coefficient, circuit-unitarity,
block-correctness, final-extraction, normalizer, or external primitive flag is
promoted.

## Current Run Directive: 2026-06-13 Post-Lower2 Source-Prepared Finite Composition Frontier

This directive supersedes the current-run evaluated backend-fold frontier for
run `20260613-045026-QBE-AUTO-002-cycle01`.  Lower2 and lower3 classified the
direct H-free attack on

```lean
oneTermRobinGamma3BoundaryEvaluatedBackendFoldStatement_n3 env
```

as a route-shape failure: the uncast active side exposes the H-free active
seven-gate `[0,0]` entry, while the theorem-facing backend/source-prepared fold
collapses through the selected prepared slot.  The evaluated fold remains an
open recovery root, but it is no longer the default next lower2 target.

The active lower target moves back to the source-prepared finite composition
field:
...
Lower2 may instead prove one strictly smaller source-shaped finite
`CircuitMatrixSemantics` or `Coeff.evalWith` theorem that directly feeds one of
these fields.  A proof of this source-prepared field then feeds the evaluated
backend fold through the existing clean-column route
`oneTermRobinGamma3BoundaryEvaluatedBackendFoldStatement_of_activePreparedEval_n3
H env hUniform`, where `hUniform` is the existing contract
`oneTermRobinGamma3BoundaryHWKappaUniformColumnAllSlotsStatement_n3 H`.
...
| Declaration | Status |
|---|---|
| `oneTermRobinGamma3BoundarySourcePreparedProjectionTarget_activeEval_iff_uncastActivePreparedCompositeEval_n3 H env` | compiled wrapper/cast removal for the source-prepared field |
| `oneTermRobinGamma3BoundaryActivePreparedCompositeEval_iff_sparseEval_n3 H env` | compiled prepared singleton/sparse clean-entry alignment |
| `oneTermRobinGamma3BoundaryActivePreparedCompositeEval_of_entryTarget_n3 H env hEntry` | compiled route from the cached entry target to the source-prepared field |
| `oneTermRobinGamma3BoundaryPreparedCompositeCleanEntryEval_eq_backend_n3 H env hUniform` | compiled prepared clean-entry backend bridge under the explicit clean-column contract |
| `oneTermRobinGamma3BoundaryEvaluatedBackendFoldStatement_of_activePreparedEval_n3 H env hUniform hActive` | compiled conditional recovery of the evaluated fold after the source-prepared field is proved |
| `oneTermRobinGamma3BoundaryUncastPreparedSparseCleanEntryEval_iff_evaluatedBackendFold_n3 H env hUniform` | compiled route wiring; retired as a lower target |
| `oneTermRobinGamma3BoundaryUnitaryEntry_eq_backendFold_n3` and `oneTermRobinGamma3BoundaryEvalGateMatrices_eq_sevenGateMatrix_n3` | diagnostic `sorry` route; not theorem closure |
...
|---|---|---|---|
| GHL2025 Eq. `arbitrary sparcity` and Shukla--Vedula 2024 | clean-column behavior of `H_W^(kappa)` | external cited contract | keep as `hUniform`; do not formalize Shukla--Vedula here |
| GHL2025 Eq. `ROBIN clarified` and Fig. `fig:1 term ROBIN` | source-prepared gamma3 branch through the prepared sandwich | GHL-internal branch/circuit transcript plus QBE-local matrix semantics | target the source-prepared field, not the standalone H-free slot reassignment |
| GHL2025 Definition `def:block-encoding` | clean projection entry compared with the prepared/backend fold | QBE-local finite projection/composition theorem | active lower work |

Lower-agent split:

| Lower profile | Required behavior |
|---|---|
| lower 1: natural-language proof architect | Append only a narrow postscript naming `source_prepared_finite_composition_leaf`, explaining the dependency path from the source-prepared field through `hUniform` to the evaluated backend fold, and retiring the direct H-free evaluated-fold route as the default target. |
| lower 2: Lean implementation worker | Edit only `QuantumBlockEncoding/RobinMatrix.lean`.  Prove the source-prepared active/prepared field above, one equivalent target above, or one strict finite composition theorem feeding it directly. |
| lower 3: necessary-condition verifier | Check source-shaped active/prepared composition/projection routes only.  Use `symbolic_bridge_gap` for source-shaped finite algebra blockers, `source_translation_gap` for an arbitrary-`H` target that needs middle restatement or an independence theorem, and `shape_or_register_gap` for standalone H-free reassignment. |

Retired routes remain retired: obstruction handles, feeder equivalences,
compiled bridge rediscovery, backend slot vanish/support work, raw `Coeff`
constructor equalities, branch-sum wrappers, H-free active-selected
diagnostics, and direct H-free evaluated-fold closure through the diagnostic
seven-gate route.
