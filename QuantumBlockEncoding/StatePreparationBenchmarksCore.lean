...
preparation targets.  Each benchmark closes the same three obligations:
normalization, full-unitary completion, and the exact state-action equation
`U |0^n> = |psi>`.

Circuit synthesis and resource comparisons live in a separate primitive-route
module.  Keeping the layers separate prevents a resource transcript from being
mistaken for a proof that the transcript implements the certified unitary.
-/
...
theorem bellMatrix_unitary :
  ...
  fin_cases row <;> fin_cases column <;>
    simp [bellMatrix, bellAmplitude, TextbookStatePreparation.invSqrtTwo,
      _root_.Matrix.mul_apply, Finset.sum_fin_eq_sum_range,
...
/-! ## Dense two-qubit benchmark for uniformly controlled rotations

The amplitudes are chosen from nested Pythagorean triples.  The first branch
has norm `5/13` with conditional amplitudes `3/5,4/5`; the second has norm
`12/13` with conditional amplitudes `5/13,12/13`.  Hence all rotation cosines
used by the primitive route are rational while the state remains dense and
non-product.
-/
...
/-- Tensor product of the `(3,4)/5` real rotation with itself. -/
...
The support stays on the little-endian `q0 = 0` plane.  Nested Pythagorean
triples give amplitudes `(3,4,12)/13`, so the pruned primitive route later uses
only rotations with rational cosines `5/13` and `3/5`.
-/
...
This is deliberately a resource-formula transcription.  It is not a proof of
the paper's coherent lookup construction, dirty-qubit semantics, approximation
bound, or asymptotic optimality theorem.
-/
