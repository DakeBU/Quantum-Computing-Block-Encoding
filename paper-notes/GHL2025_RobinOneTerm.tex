...
The matrix form is a permutation on $2^{\mathrm{totalQubits}}$:
the image replaces the system register bits while preserving the sparse index and all other bits.

In Lean:
\[
  \texttt{GHL2025.robinSparseColumnMap} \quad (\text{column mapping}),
  \qquad
  \texttt{GHL2025.bandedSparseAccessMatrix} \quad (\text{full permutation}).
\]
The unitarity obligation remains unproved ($\texttt{proved := false}$).

\subsection{$(O_D^{BS})^\dagger$ Inverse Matrix}

For a permutation matrix $P$ where $P[\mathrm{image}(j)][j]=1$, the Hermitian
conjugate is the transpose $P^\dagger[j][\mathrm{image}(j)]=1$.
The Lean implementation computes $\mathrm{image}(i)$ for the row index
and checks whether $j = \mathrm{image}(i)$.
...
\(O_D^{BS}\), \(O_f\), SWAP, and \((O_D^{BS})^\dagger\).  These matrices
are still proof-obligation carriers, not completed correctness proofs:
