...
The banded sparse access oracle (Lemma 1, main.tex:784--801) maps
\[
  O_D^{BS} |s\rangle|i\rangle = |s\rangle|\mathrm{col}(s,i)\rangle
\]
where $\mathrm{col}(s,i)$ is the column index of the $s$-th nonzero entry in
row $i$ of the Robin derivative matrix, ordered by offset from left to right.
...
The matrix form replaces the system register bits while preserving the sparse index and all other bits.
...
\[
  ...
  0.
\]
Thus the permutation-matrix proof route is blocked until the paper's sparse-access oracle contract is reconciled with the Robin boundary rows.  The unitarity obligation remains unproved ($\texttt{proved := false}$).

\subsection{$(O_D^{BS})^\dagger$ Inverse Matrix}

The Lean implementation records a transpose-style matrix by computing
$\mathrm{image}(i)$ for the row index and checking whether
$j = \mathrm{image}(i)$.  The full inverse-permutation statement is blocked
by the same boundary-row issue as the forward sparse-access matrix.
