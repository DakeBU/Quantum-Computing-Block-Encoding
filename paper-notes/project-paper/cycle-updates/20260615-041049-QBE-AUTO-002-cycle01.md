...
Generated: `2026-06-15 04:25:49`

Run directory: `runs/20260615-041049-QBE-AUTO-002-cycle01`
...
- 1. lower1 validates the source proof map in the finite-normalized-projection middle packet and keeps the branch fixed to system entry `(0,0)`, sparse slot `2`, source-prepared projection, and normalizer `N_D*N_f*kappa`.
- 2. lower3 verifies the necessary conditions before lower2 edits Lean: signal block indices are `[0,0]`, branch basis is `[32,32]`, the conditional bridge consumes the source-prepared projection entry, and raw backend expansion plus `SignalEntryFold` equival...
- 3. lower2 may edit only `QuantumBlockEncoding/RobinMatrix.lean` and only for the declaration above. If it already exists, lower2 should make no Lean edit and log `error_class=stale_leaf`.
...
| --- | --- | --- | --- | --- |
| conditional_normalizer_product_bridge | stale_leaf | True | True | prepare finite projection/summation proof packet for oneTermRobinGamma3ProductToCoefficientObligation 3 0 0; do not revive backendExpansionStatement or SignalEntryFold |
| conditional_normalizer_product_bridge | None | True | True | Use the compiled conditional bridge as route memory; next proof work must target the remaining finite normalized-block/projection bridge for oneTermRobinGamma3ProductToCoefficientObligation 3 0 0, not the retired backendExpansionStatement or SignalEntryFold route. |
| conditional_normalizer_product_bridge | None | True | True | prove finite normalized-block/projection bridge for oneTermRobinGamma3ProductToCoefficientObligation 3 0 0; do not revive backendExpansionStatement |
...
## 2026-06-15 04:13:51 - upper

Upper synthesis: mode faithful paper reproduction. Objective is a source-proof translation packet for the finite normalized-block/projection bridge feeding oneTermRobinGamma3ProductToCoefficientObligation 3 0 0. Route memory: oneTermRobinGamma3BoundaryHWKappaUniformAllSlots_to_productRouteFocusedCleanColumn_n3, oneTermRobinGamma3BoundarySourcePreparedSlot2Product_normalizerEval_n3, and oneTermRobinGamma3BoundarySourcePreparedProjectionSlot2Product_feedsFixedProductMap_n3 are compiled and stale as lower targets. No-go: oneTermRobinGamma3BoundaryBackendBranchContributionTarget_n3.backendExpansionStatement, SignalEntryFold-equivalent raw folds, and the diagnostic sorry route are forbidden by oneTermRobinGamma3BoundaryBackendExpansionStatement_not_n3. Middle/lower1 should audit the source projection step from slot-2 branch product [32,32] to signal-zero block entry [0,0] and name exactly one Lean declaration before lower2 edits. Lower3 should check branch/register shape, finite matrix support, normalizer hypotheses, and product_to_coefficient=false. All oracle, H_W, R_y, LCU, normalized-block, block-correctness, final-extraction, resource, and product-to-coefficient flags remain false.
```
