...
const robinTarget = `Encode the fixed N = 8 homogeneous-Robin fourth-order second-derivative matrix from tasks/QBE-ROBIN-BE-COLD-001.md. Preserve alpha = 56/3, epsilon = 0, signal index zero, explicit register order, and the lexicographic score (semantic tier, gate count, depth, auxiliary qubits, oracle calls).`;

function applyBenchmarkPreset() {
  const preset = benchmarkPreset.value;
  if (preset === "custom") return;
  const cold = preset === "robin-cold";
  taskName.value = cold ? "QBE-ROBIN-BE-COLD-001" : "QBE-ROBIN-BE-WARM-001";
  mode.value = "exploratoryConstruction";
  oracleDescription.value = robinTarget;
  normalizer.value = "56/3";
  projector.value = "signal index 0; system register Fin 8";
  baseline.value = cold
    ? "No Robin-specific construction or proof memory is visible in this arm."
    : "Use the Guseynov-Huang-Liu sparse-register, indicator, derivative-amplitude, boundary-rotation, sparse-access, coefficient, swap, cleanup, and inverse-preparation construction as the seed; certify it before ranking mutations.";
  constraints.value = cold
    ? "Evaluation mode: isolated-abeis. Do not read GHL2025.lean, RobinHeat.lean, RobinMatrix.lean, paper notes, old Robin runs, or compiled declaration indexes."
    : "Evaluation mode: full-abeis. Reproduce the source seed at one explicit semantic tier. Retire the compiled false H-free raw-fold route. Compare only candidates expanded to the same gate/resource convention.";
  exportQiskit.checked = true;
  buildPacket();
}
...
function runnerEnvTemplate() {
  ...
  return `ASPBE_RUN_LOCATION=${runLocation.value}
ASPBE_API_PROVIDER=${provider}
ASPBE_DEFAULT_MODEL=${model}
ASPBE_RUNNER_ENDPOINT=${endpoint}
# Set this in your shell or self-hosted runner secret store, not in Git:
${keyName}=...`;
}

function localRunnerCommand(title) {
  if (benchmarkPreset.value === "robin-cold" || benchmarkPreset.value === "robin-warm") {
    const arm = benchmarkPreset.value === "robin-cold" ? "cold" : "warm";
    return `# Replays the frozen Robin benchmark in its own detached worktree.
python3 tools/run_robin_repro.py prepare --arm ${arm}
python3 tools/run_robin_repro.py run --arm ${arm} --cycles 1
...
function buildPacket() {
  ...
  const md = `# ASPBE Task Packet: ${title}

Generated by the static web task builder at ${date}.

- Reproduction preset: \`${benchmarkPreset.value}\`
...
write the same ASPBE artifacts and the same Lean gate output.

## Agent Backend Preferences

The web page records the user's preferred model backend for each ASPBE role.
...
async function runWithApi() {
  ...
  const payload = {
    ...
    task: {
      ...
      reproductionPreset: benchmarkPreset.value,
...
benchmarkPreset.addEventListener("change", applyBenchmarkPreset);
