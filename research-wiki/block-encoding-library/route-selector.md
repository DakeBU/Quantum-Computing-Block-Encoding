...
For the cross-cutting question **“under this exact query/access model, should
this SP/BE task plausibly be constructible at all?”**, first read
[`research-wiki/construction-methodology/index.md`](../construction-methodology/index.md).
The common planner test is whether the representation needed by the route can
be **computed coherently, used with explicit precision/normalization, and
uncomputed with the promised workspace restored**. This is a route-selection
principle inferred from recurring constructions, not a completeness theorem.
...
Before route search, freeze the target, register order, clean/dirty ancillas,
query interfaces, normalization $\alpha$, error $\varepsilon$, and resource
priority. A classically simple matrix is not automatically cheap to block
encode: the true bottleneck can be data loading, reversible arithmetic,
condition number, normalization, precision, or uncomputation.
...
| Compiled Lean leaf | theorem already available in ASPBE | instantiate it or write a small adapter; do not reprove it |
...
| interval/prefix/piecewise arithmetic structure | reversible comparator/incrementer routing | inherited from routed values/blocks | exact predicate permutation, primitive refinement, clean restoration, then clean block | comparison/addition cost dominates or the arithmetic access model is unstated |
...
> T / Toffoli / T-depth when the primitive model makes them meaningful
> lower proof burden and reusable leaves
```

Always retain $\alpha$ and $\varepsilon$ in the comparison record. A circuit
with fewer gates but a changed normalization, error tolerance, or query model is
not an unqualified improvement of the original candidate.
...
of reproving their branch algebra. Upper must keep the original candidate as a
baseline; middle opens separate leaves for the promise predicate,
compute-uncompute restoration, involution before any dirty-ancilla variant, and
a same-tier resource comparison. Until those leaves compile, the mutation
remains exploratory.

## Route 12: Reversible Arithmetic / Interval Routing

Use when matrix entries or nonzero locations are described by inequalities,
prefixes, modular shifts, band offsets, or piecewise regions. The reusable
pattern is
...
\;\xrightarrow{\mathrm{use}}\;
|x\rangle|p(x)\rangle_f|F_{p(x)}(y)\rangle
\;\xrightarrow{\mathrm{uncompute}}\;
|x\rangle|0\rangle_f|F_{p(x)}(y)\rangle.
$$

Retrieve the comparator/incrementer arithmetic memory before synthesizing a
new predicate circuit. ASPBE now has finite proof-bearing seeds for a three-bit
incrementer, a two-bit `< 3` comparator, and a clean interval selector together
with exact refinement to the repository's `{X, RY, RZ, CX}` primitive basis.
The source paper's arbitrary-width asymptotic optimality theorem remains a
separate formalization frontier until its recursive construction and lower
bounds are reproduced.

This route is shared with State Preparation: the same comparator can steer an
interval tree or sparse support loader, so the proof graph should expose one
arithmetic dependency rather than duplicate it under SP and BE.

## Constructability warnings

The route selector is not a theorem that the listed access models are necessary
or sufficient for all efficient block encodings. In particular:

- failure under one query model does not rule out a better representation;
- “sparse” is not enough without a coherent support/value interface;
- an efficient classical formula is not enough until it is compiled
  reversibly with stated precision and cleanup;
- an existence dilation does not imply an efficient gate-level circuit;
- “optimal” requires a named gate/query/ancilla model and a matching lower
  bound. Otherwise say “best certified in this search tier” or “Pareto-better
