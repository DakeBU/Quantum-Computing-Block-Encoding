...
2. Record the source with a stable link, paper title, authors, arXiv version if
...
3. Classify the status:
   ...
   - `formalized`: QBE has a build-tested Lean declaration for the used
     statement.
4. Add the Lean declaration if it exists, or a planned declaration if not.
5. Add dependent QBE tasks, gates, proof-DAG blocks, and proof obligations.
6. If a lower agent wants to use the result, check that its status is
